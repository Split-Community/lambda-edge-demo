...
- **Do not commit** `terraform/terraform.tfvars` (it contains `split_sdk_key`). It is listed in `.gitignore`.
- **Do not commit** `edge/config.json`; it is generated by Terraform and contains the SDK key. It is in `.gitignore`.
- **Do not commit** `terraform/*.tfstate` or `*.tfstate.*`; they may contain account IDs and resource ARNs. They are in `.gitignore`.
- If you ever committed secrets, rotate the Split SDK key and remove the files from git history.

## Terraform inputs

| Variable | Description | Default |
|----------|-------------|---------|
| `split_sdk_key` | Split server-side SDK key | (required) |
| `feature_flag_name` | Split feature flag name (used by edge) | `my_feature` |
| `project_name` | Prefix for resource names (e.g. Lambda, table) | `edge-redirect` |
| `aws_region` | Primary region for DynamoDB and sync Lambda | `us-east-1` |
| `sync_schedule_rate_minutes` | How often the sync Lambda runs | `5` |
...
- `PLAN.md` – Design and mapping from the Cloudflare template.

## Lambda@Edge package size

The edge Lambda includes the Split browser SDK and can approach the **1 MB compressed** limit. If deploy fails with a package-too-large error, use a bundler (e.g. esbuild) to tree-shake and produce a smaller zip; the handler and wrapper logic stay the same.
